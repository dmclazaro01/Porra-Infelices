...
export { SKILLS_MAP, COMBO_SKILLS_MAP, FRONTEND_PACKAGES, FRONTEND_BONUS_SKILLS, WEB_FRONTEND_EXTENSIONS, AGENT_FOLDER_MAP, } from "./skills-map.js";
import { SKILLS_MAP, COMBO_SKILLS_MAP, FRONTEND_PACKAGES, FRONTEND_BONUS_SKILLS, WEB_FRONTEND_EXTENSIONS, AGENT_FOLDER_MAP, } from "./skills-map.js";
// ── Internal Constants ───────────────────────────────────────
    ...
    function scan(dir, depth) {
        ...
        for (const entry of entries) {
            if (entry.isFile()) {
                ...
                if (normalizedExtensions.some((ext) => lowerName.endsWith(ext)))
                    return true;
            }
    ...
    function scan(dir, depth) {
        ...
        for (const entry of entries) {
            if (entry.isFile()) {
                ...
                if (dot !== -1 && WEB_FRONTEND_EXTENSIONS.has(name.slice(dot)))
                    return true;
            }
            else if (entry.isDirectory() && depth < maxDepth) {
                if (SCAN_SKIP_DIRS.has(entry.name) || entry.name.startsWith("."))
...
export function resolveWorkspaces(projectDir, preloaded) {
    ...
    if (existsSync(pnpmPath)) {
        try {
            ...
            if (patterns.length > 0) {
                return expandWorkspacePatterns(projectDir, patterns).filter((d) => resolve(d) !== resolve(projectDir));
            }
        }
        catch { }
    }
    ...
    if (denoJson?.workspace) {
        ...
        if (members.length > 0) {
            return expandWorkspacePatterns(projectDir, members).filter((d) => resolve(d) !== resolve(projectDir));
        }
    }
    return [];
}
