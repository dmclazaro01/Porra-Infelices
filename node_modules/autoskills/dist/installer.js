...
let _cachedRegistry;
let _cachedRegistryDir = null;
let _cachedPackageVersion;
...
export function _setRegistryDir(dir) {
...
export function verifyRegistryEntry(skillName, entry, registryDir = getRegistryDir()) {
...
function relPathFromTo(from, to) {
    const rel = relative(from, to);
    return rel.split("\\").join("/");
}
function normalizeRegistryRelPath(rel) {
    return rel.split("\\").join("/");
}
function sha256Buffer(buf) {
    return createHash("sha256").update(buf).digest("hex");
}
function getRegistryRawBaseUrls(opts) {
...
function getInstallRegistryDir(opts) {
    return opts.registryDir || getRegistryDir();
}
export function getAutoskillsCacheDir() {
    return (process.env.AUTOSKILLS_CACHE_DIR || join(homedir(), ".cache", "autoskills", "skills-registry"));
}
export function clearAutoskillsCache() {
    const cacheDir = getAutoskillsCacheDir();
    const removed = existsSync(cacheDir);
    rmSync(cacheDir, { recursive: true, force: true });
    return { cacheDir, removed };
}
function getCacheRegistryDir(entry) {
    return join(getAutoskillsCacheDir(), entry.bundleHash);
}
function securityCheckForEntry(skillName, entry) {
...
export function securityCheckForSkillPath(skillPath) {
...
function encodeRawPath(skillName, rel) {
    return [skillName, ...normalizeRegistryRelPath(rel).split("/")].map(encodeURIComponent).join("/");
}
function githubDownloadHeaders(url) {
    const headers = { "User-Agent": "autoskills" };
...
function isDisallowedSkillFile(rel) {
    return rel.toLowerCase().endsWith(".zip");
}
async function downloadRegistryFile(skillName, entry, rel, opts) {
    const normalizedRel = normalizeRegistryRelPath(rel);
    ...
    for (const baseUrl of getRegistryRawBaseUrls(opts)) {
        ...
        if (!res.ok) {
            ...
            if (res.status === 403 && res.headers.get("x-ratelimit-remaining") === "0") {
                throw new Error(`GitHub rate limit exceeded${resetSuffix}. Set GITHUB_TOKEN or GH_TOKEN to increase the limit.`);
            }
...
async function downloadRegistryEntry(skillName, entry, destDir, opts) {
    const files = [];
    for (const rel of entry.files) {
        files.push({
...
function copyRegistryEntryFromLocal(skillName, entry, destDir, opts) {
...
function copyRegistryEntryFromCache(skillName, entry, destDir, opts) {
...
async function downloadRegistryEntryToCache(skillName, entry, opts) {
    const registryDir = getCacheRegistryDir(entry);
    const skillDir = join(registryDir, skillName);
...
function ensureSymlinkTo(target, linkPath) {
...
export function agentFolderFor(agent) {
...
function updateSkillsLock(projectDir, skillName, entry) {
...
export async function installSkill(skillPath, agents = [], opts = {}) {
    ...
    const { skillName } = parseSkillPath(skillPath);
    if (!skillName)
        return fail(`invalid skill path: ${skillPath}`);
    opts.onTrace?.(`resolving ${skillPath}`);
    const registry = loadRegistry();
    if (!registry) {
        return fail(`skills-registry index not found. Run 'pnpm sync:skills' in the autoskills package.`);
    }
    const entry = registry.skills[skillName];
    if (!entry) {
    ...
    try {
        ...
        return fail(`download failed: ${err.message}`);
    }
    ...
    for (const folder of uniqueFolders) {
        ...
        try {
            ...
            symlinkErrors.push(`${folder}: ${err.message}`);
        }
    }
    try {
        ...
        return fail(`lockfile update failed: ${err.message}`);
    }
...
export async function installAll(skills, agents = [], opts = {}) {
    if (opts.verbose)
        return installAllVerbose(skills, agents, opts);
    if (!process.stdout.isTTY)
        return installAllSimple(skills, agents, opts);
    const CONCURRENCY = 6;
        ...
        while (nextIdx < total) {
            ...
            if (result.success) {
                ...
                if (result.securityCheck)
                    securityChecks.push(result.securityCheck);
            }
            else {
                state.status = "failed";
...
async function installAllVerbose(skills, agents = [], opts = {}) {
    const sorted = sortByRepo(skills);
    let installed = 0;
    let failed = 0;
    const errors = [];
...
async function installAllSimple(skills, agents = [], opts = {}) {
    const CONCURRENCY = 6;
    const sorted = sortByRepo(skills);
    let installed = 0;
    let failed = 0;
