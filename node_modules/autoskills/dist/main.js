...
import { log, write, bold, dim, green, yellow, cyan, magenta, red, pink, gray, muted, SHOW_CURSOR, } from "./colors.js";
import { printBanner, multiSelect, formatTime } from "./ui.js";
import { clearAutoskillsCache, installAll, loadRegistry, securityCheckForSkillPath, } from "./installer.js";
...
function securityWarningForSkill(skill) {
...
function printSkillsList(skills) {
    ...
    const maxEffective = Math.max(...entries.map((e) => e.label.length +
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        const effectiveLen = label.length +
...
function formatSecurityFindings(check) {
    const findings = check.findings.map((finding) => finding.trim()).filter(Boolean);
...
function printSecurityChecks(checks) {
    ...
    log();
    log(dim(`   | ${visiblePad("Skill", skillWidth)} | ${visiblePad("Check", checkWidth)} | ${visiblePad("Findings", findingsWidth)} |`));
    ...
    for (const { check, findings } of sorted) {
        ...
        log(`   | ${visiblePad(truncateVisible(check.name, skillWidth), skillWidth)} | ${visiblePad(status, checkWidth)} | ${visiblePad(lines[0], findingsWidth)} |`);
...
function printSummary({ installed, failed, errors, elapsed, verbose }) {
    log();
    if (failed === 0) {
        log(green(bold(`   ✔ Done! ${installed} skill${installed !== 1 ? "s" : ""} installed in ${formatTime(elapsed)}.`)));
    }
    else {
        log(yellow(`   Done: ${green(`${installed} installed`)}, ${red(`${failed} failed`)} in ${formatTime(elapsed)}.`));
        if (errors.length > 0) {
            log();
            log(bold(red("   Errors:")));
...
async function selectSkills(skills, autoYes) {
    ...
    const maxEffective = Math.max(...skills.map((s) => {
        const cached = labelCache.get(s.skill);
        return (cached.label.length +
    ...
    const selected = await multiSelect(skills, {
        ...
        shortcuts: installedCount > 0
            ? [
                { key: "n", label: "new", fn: (items) => items.map((s) => !s.installed) },
                {
                    key: "i",
                    label: "installed",
                    fn: (items) => items.map((s) => s.installed),
                },
            ]
            : [],
    });
